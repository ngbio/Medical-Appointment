...
#### 3.4.2.1. ADR-05: Relational Database for Persistent Data Storage
...
Do đó, hệ thống bắt buộc phải sử dụng một giải pháp cơ sở dữ liệu có khả năng:
  ...
  * Quản lý tốt các mối quan hệ giữa các luồng dữ liệu.
...
Phía Backend được xây dựng bằng **Django**, sử dụng **Django ORM** để tương tác trực tiếp với MySQL. Việc sử dụng Django ORM cho phép ánh xạ tự động các object/model trong code sang các bảng trong database.
...
#### 4\. Cơ sở phân tích và lựa chọn

Quyết định chọn MySQL được đưa ra dựa trên 4 lý do cốt lõi sau:

*  **Phù hợp tuyệt đối với dữ liệu quan hệ:** Hệ thống mạng lưới dữ liệu dày đặc (Patient → Appointment, Doctor → Appointment, Appointment → MedicalRecord). Database quan hệ giúp quản lý các Foreign Keys và cấu trúc này một cách tối ưu nhất.
...
*  **Tương thích hoàn hảo với Backend Framework:** Django ORM được thiết kế để làm việc cực kỳ trơn tru với MySQL. Nó cho phép định nghĩa các Models bằng Python, tự động generate database schema, và thực hiện các câu lệnh query phức tạp dễ dàng mà không cần viết SQL raw.
